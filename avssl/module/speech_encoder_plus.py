...
class S3prlSpeechEncoderPlus(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.name.startswith("hubert") | self.name.startswith("wavlm"):
            if (
...
class Custom_WavLM(nn.Module):
    MODEL2PATH = {
        "wavlm_base": "/mnt/md0/user_jeff/wavlm_pt/WavLM-Base.pt",
        "wavlm_base_plus": "/mnt/md0/user_jeff/wavlm_pt/WavLM-Base+.pt",
        "wavlm_large": "/mnt/md0/user_jeff/wavlm_pt/WavLM-Large.pt",
    }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert name in self.MODEL2PATH, "Model name({}) should be in {}".format(
            name, self.MODEL2PATH.keys()
        )
        ...
        checkpoint = torch.load(self.MODEL2PATH[self.name])
        self.cfg = WavLMConfig(checkpoint["cfg"])
        self.encoder = WavLM(self.cfg)
        self.encoder.load_state_dict(checkpoint["model"])

        logger.info(f"Normalize waveform = ({self.cfg.normalize})")
    ...
    def forward(
        ...
        # for i, feat in enumerate(features["layer_results"]):
        ...
        if feat_select_idx == "all":
            ...
            feat = [feat["hidden_states"][i] for i in feat_select_idx]
            return_list.extend([feat, feat_len])
        elif isinstance(feat_select_idx, int):
            feat = feat["hidden_states"][feat_select_idx]
            return_list.extend([feat, feat_len])
        elif feat_select_idx in feat:
            return_list.extend([feat[feat_select_idx], feat_len])
        else:

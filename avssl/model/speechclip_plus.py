...
class GeneralBase(BaseLightningModel):
    ...
    def forward_audio(
        ...
        if self.training:
            random_crop_max_length(batch, self.config.audio_encoder.max_audio_len)
            
        if self.audio_encoder_type in ["s3prl_plus", "FairseqHubert", "custom_wavlm"]:
            return self.audio_encoder(batch, return_hidden_states=return_hidden_states)
        else:
    ...
    def validation_epoch_end(self, outputs, log_cos_semantics=False):
        ...
        if "vq_keywords" in outputs[0].keys():
            ...
            if not os.path.exists(os.path.join(RootDir, "visualization")):
                os.makedirs(os.path.join(RootDir, "visualization"), exist_ok=True,)
            ...
            if (
                ...
                if self.keyword_num is None: all_keyword_embeddings.squeeze(1)  
                ...
                ## Drawing PCA ##
                #################
                self.log_draw_pca_every_n_epoch = getattr(self.config.log_setting, "log_draw_pca_every_n_epoch", 0)
                if self.log_draw_pca_every_n_epoch > 0 and (self.current_epoch % self.log_draw_pca_every_n_epoch == 0):
                ...
                for x in outputs:
                    ...
                    if self.keyword_num is None:
                        ...
                        embdList = torch.split(x["vq_keywords"], (x["keywords_bsz"] * x["max_kw_num"]).tolist(), dim=0)
                        keyword_embeddings_list.append(
                            [ embd.view(bsz, knum, -1) for bsz, knum, embd in zip(x["keywords_bsz"], x["max_kw_num"], embdList)]
                        )

                ####################################
                ## Retreiving keywords' neighbors ##
                ####################################
                retrieve_method = getattr(self.config.model_settings.cascaded_branch.keyword, "retrieve_method", "cosine")
                if retrieve_method not in ["cosine", "pseudo_inverse"]: raise NotImplementedError(retrieve_method)
                ...
                if self.keyword_num is not None:
                    ...
                    hit_rate_list, kw_top_ret, all_retok_outputs = compute_fixed_keyword_neighbors(
                                                                model=self,
                                                                K=K,
                    ...
                    hit_rate_list, all_retok_outputs = compute_dynamic_keyword_neighbors(
                                                        model=self,
                                                        K=K,
                                                        retreival_type=retrieve_method,
                                                        outputs=outputs,
                ...
                with open(os.path.join(TextDir, f"keywords_ep{self.current_epoch}.json"), "w") as f_kw:
        ...
        if hasattr(self.clip, "original2Reduced"):
            for i in range(textTensor.shape[0]):
                for j in range(textTensor.shape[1]):
                    textTensor[i, j] = self.clip.original2Reduced[textTensor[i, j].item()]
...
class SpeechCLIP_plus(GeneralBase):
    def __init__(self, config: OrderedNamespace):
        super().__init__(config)
        self.dataset_name = config.data.dataset.name
        assert self.dataset_name in ["flickr", "coco"], self.dataset_name
        ...
        if self.config.model_settings.cascaded_objective_weight > 0:
            ...
            elif self.config.model_settings.cascaded_branch.type == "HybridBrnach_plus":
                assert self.config.model_settings.parallel_objective_weight > 0, self.config.model_settings.parallel_objective_weight
                logger.info("Using Parallel Objective (Integrated w/ cascaded_branch) and dynamic numbers of keywords")
            ...
            elif self.config.model_settings.cascaded_branch.type == "HybridBranch_dynamic":
                assert self.config.model_settings.parallel_objective_weight > 0, self.config.model_settings.parallel_objective_weight
            ...
            if (self.config.model_settings.cascaded_branch.downsampling.type == "cif"):
                self.quantity_loss_weight = getattr(self.config.model_settings.cascaded_branch.downsampling.cif, "quantity_loss_weight", 1.0)
                self.quantity_loss_criteria = nn.L1Loss()

        if (
            self.config.model_settings.parallel_objective_weight > 0
            and not self.config.model_settings.cascaded_branch.type == "Hybrid_plus"
        ):
        ...
        if self.keyword_objective_weight > 0:
            logger.info("Adding keyword contrastive loss objective")
        ...
        if self.keyword_diversity_weight > 0:
            logger.info(f"Adding keyword diversity objective, weight: {self.keyword_diversity_weight}")
            ...
            assert isinstance(self.keyword_diversity_type, list), type(self.keyword_diversity_type)
            
            logger.info(f"Keyword diversity type: {self.keyword_diversity_type}")
            assert set(self.keyword_diversity_type).issubset({"cos", "corr", "ent"}), f"The implemented types of diversity loss are ['cos', 'corr', 'ent'], your target diversity type: {self.keyword_diversity_type}"
            
            if ( "corr" in self.keyword_diversity_type or "cos" in self.keyword_diversity_type ):
        ...
        if self.parallel_branch is not None:
            if not hasattr(self.parallel_branch, "self_att"):
                self.attention_diversity_weight = 0
            else:
                self.attention_diversity_weight = (
                    config.model_settings.parallel_branch.transformer_args.get("attn_diversity_weight", 0.0)
                )
        else:
            if not hasattr(self.cascaded_branch, "self_att"):
                ...
                self.attention_diversity_weight = (
                    config.model_settings.cascaded_branch.transformer_args.get("attn_diversity_weight", 0.0)
                )

        if self.attention_diversity_weight > 0:
            logger.info(f"Adding attention diversity loss, weight: {self.attention_diversity_weight}")
    ...
    def forward(self, batch,) -> dict:
        ...
        ## Extract features ##
        ######################
        image_feat = self.forward_image(batch["image"])
        if self.img_enc_proj_net is not None:
        ...
        if self.cascaded_branch is not None:
            ...
            if "segment_num" in batch:
                inputDict["dsample_target_len"] = batch["segment_num"]
            else:
                if getattr(self.cascaded_branch, "using_gt_len", False) and "text" in batch:
                    inputDict["text"] = batch["text"]
                    dsample_target_len = [ (t.squeeze().tolist().index(49407) - 1) for t in batch["text"] ]
                else: 
                    inputDict["text"] = None
                    dsample_target_len = [ int(_l.item() / 20) for _l in audio_feat_len ]
                    
                inputDict["dsample_target_len"] = torch.LongTensor(dsample_target_len).to(audio_feat.device)
                
            resultDict = self.cascaded_branch(inputDict)
            assert "vq_results" in resultDict and "vq_keywords" in resultDict, f"{resultDict.keys()}"
            vq_results = resultDict["vq_results"]
            vq_keywords = resultDict["vq_keywords"]
            
            if not isinstance(self.cascaded_branch, CascadedBranch) and not isinstance(self.cascaded_branch, HybridBranch):
                assert "dsample_results" in resultDict and "dsample_feats_length" in resultDict, f"{resultDict.keys()}"
                dsample_dict = resultDict["dsample_results"]
                keywords_len = dsample_dict["dsample_feats_length"]
            else:
        ...
        resultDict["image_feat"] = image_feat
        resultDict["keywords_len"] = keywords_len
        
        ####################
        ## Extract losses ##
        ####################
        parallel_audio_feat = resultDict.get("parallel_audio_feat", None)
        cascaded_audio_feat = resultDict.get("cascaded_audio_feat", None)
        assert cascaded_audio_feat is not None or parallel_audio_feat is not None, "Provide at leats one type of features to compute the loss"
        ...
        if self.cascaded_branch is not None:
            if self.keyword_diversity_weight > 0:
                assert vq_keywords is not None, f"{resultDict.keys()}"    
                    ...
                    if div_type == "ent":
                        ...
                        if self.keyword_num is not None:
                            keyword_diversity_loss = self.keyword_diversity_criterion(
                                vq_keywords, [self.keyword_num] * vq_keywords.shape[0], div_type
                            )
                        else:
                            keyword_diversity_loss = self.keyword_diversity_criterion(
            ...
            if self.keyword_objective_weight > 0 and "proj_keywords" in resultDict:
                keywords_audio_feat = torch.mean(resultDict["proj_keywords"], dim=1)
                keywords_audio_feat = keywords_audio_feat / keywords_audio_feat.norm(
                    dim=-1, keepdim=True
                )
                losses["keywords_audio_feat"] = keywords_audio_feat
                
            if hasattr(self.cascaded_branch, "downsampling_type") and self.cascaded_branch.downsampling_type == "cif":
                assert "target_len" in dsample_dict and "quantity_out" in dsample_dict, f"{dsample_dict.keys()}"
                losses["cif_quantity_out"] = dsample_dict["quantity_out"]
                losses["cif_target_len"] = dsample_dict["target_len"]
        ...
        if self.cascaded_branch is not None:
            if dsample_dict is not None and "dsample_len_diff" in dsample_dict:
                log_metrics["dsample_len_diff"] = dsample_dict["dsample_len_diff"]
            vq_results_log_keys = ["temp", "code_perplexity", "prob_perplexity", "ent_per_t"]
            assert set(vq_results_log_keys).issubset(set(vq_results.keys())), f"log keys: {vq_results_log_keys}, result: {vq_results.keys()}"
            vq_log_dict = { k: vq_results[k] for k in vq_results_log_keys}
            log_metrics.update(vq_log_dict)

        return losses, log_metrics, resultDict
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if "image_feat" in others: 
            return_dict["image_feat"] = image_feat
        if "text_feat" in others:
            return_dict["text_feat"] = text_feat

        if "vq_keywords" in others and others["vq_keywords"] is not None:
            vq_kw_dict = {"gold_text": batch["text"]}
            if self.keyword_num is not None:
                vq_kw_dict["vq_keywords"] = others["vq_keywords"]
            else:
                vq_keywords = others["vq_keywords"].view(-1, vq_keywords.shape[-1])
                vq_kw_dict.update({"vq_keywords": vq_keywords, 
                                    "keywords_bsz": vq_keywords.shape[0],
                                    "max_kw_num": vq_keywords.shape[1],})
                
            return_dict.update(vq_kw_dict)
    ...
    def feature_extractor_s3prl(self, wav):
        ...
        if self.parallel_branch is not None:
            additional_hidden_states = self.parallel_branch.extract_hidden_states(audio_feat, audio_feat_len)
        else:
            if hasattr(self, "self_attn"):
                additional_hidden_states = self.cascaded_branch.extract_hidden_states(audio_feat, audio_feat_len)
            else:
    ...
    def extract_keyword_boundary(self, wav, alignment):
        ...
        inputDict = {"audio_feat": embeddings, "audio_feat_len": feat_len, "fp_alignment": fp_alignment.unsqueeze(0), "dsample_target_len": segment_num }

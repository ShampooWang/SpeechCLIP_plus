...
class CascadedBranch_plus(nn.Module):
    ...
    def extract_hidden_states(self, audio_feat, audio_len, text=None, use_kw=False):
        bsz = audio_feat.shpae[0]
        key_padding_mask = get_keypadding_mask(
            max_length=audio_feat.shape[1], data_lens=audio_len
        )
    ...
    def forward(
        ...
        if self.downsampling_type == "cnn":
            keywords, new_feat_len = self.downsampling(audio_feat, audio_len)
        elif self.downsampling_type == "cif":
            if self.using_gt_len and text is not None:
                text_toks_len = []
                for t in text:
...
class Hybrid_plus(CascadedBranch_plus):
    def __init__(
    ...
    def extract_hidden_states(
        ...
        # hidden_states.append(downsampling_out)
        # return hidden_states
        downsampling_out["cif_weight"] = downsampling_out["cif_weight"].unsqueeze(-1)
        for i in range(audio_feat.shape[0]):
            audio_feat[i] = torch.mul(downsampling_out["cif_weight"][i], audio_feat[i])
    ...
    def forward(
        ...
        if self.downsampling_type == "cnn":
            audio_feat, new_feat_len = self.downsampling(audio_feat, audio_len)
        elif self.downsampling_type == "cif":
            if self.using_gt_len and text is not None:
                text_toks_len = []
                for t in text:
...
class Alignment_CascadedBranch(nn.Module):
    def __init__(
        ...
        logger.info("Using Alignment_CascadedBranch")

        assert hasattr(
            TransformerModels, config.model_settings.cascaded_branch.transformer_type
        )
        logger.info(
            f"Using {config.model_settings.cascaded_branch.transformer_type} as CascadedBranch"
        )
        self.self_att = getattr(
            TransformerModels, config.model_settings.cascaded_branch.transformer_type
...
class SpeechCLIP_plus(GeneralBase):
    def __init__(self, config: OrderedNamespace):
        ...
        if self.config.model_settings.cascaded_objective_weight > 0:
            ...
            if self.config.model_settings.cascaded_branch.type == "Cascaded_plus":
            ...
            elif self.config.model_settings.cascaded_branch.type == "Hybrid_plus":
                ...
                self.cascaded_branch = Hybrid_plus(
                    config=self.config,
                    audio_dim=self.audio_embd_dim,
                    text_dim=self.subword_embd_dim,
    ...
    def validation_epoch_end(self, outputs):
        if "vq_keywords" in outputs[0].keys():
            ...
            if (
                ...
                # cos_semantic_list = cosine_semantics(all_retok_outputs, self.dataset_name)
    ...
    def feature_extractor_s3prl(self, wav, featrure_layer_norm=True, add_cif=False):
        ...
        if add_cif:
            if self.cascaded_branch is not None:
                ...
                # hidden_states.append(cascaded_hidden_states)

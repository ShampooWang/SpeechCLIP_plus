...
class Runner():
    ...
    def get_features_norm(self):
        ...
        for wavs, *others in tqdm(dataloader, dynamic_ncols=True, desc=split):
            ...
            all_feats = torch.stack(features["hidden_states"], dim=1)
            layers_norm = torch.norm(all_feats, dim=(2,3))
            del all_feats
            mean_layers_norm = torch.mean(layers_norm, dim=0).cpu().detach().numpy()
            mean_layers_norm_mtx.append(mean_layers_norm)

            # mean_layer_norm_list = []
            # for i in range(len(features["hidden_states"])):
            #     norm = torch.norm(features["hidden_states"][i], dim=0)
            #     print(norm.size())
            #     mean_layer_norm_list.append(torch.mean(norm, dim=0).item())
            # mean_layer_norm_mtx.append(mean_layer_norm_list)

        mean_layers_norm_mtx = np.array(mean_layers_norm_mtx)
        print(mean_layers_norm_mtx.shape)
        print(list(np.mean(mean_layers_norm_mtx, axis=0)))
        return
